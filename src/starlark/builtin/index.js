...
function abs(thread, _, args, kwargs) {
    var res = new Array(1);
    var unpackError = (0, unpack_1.UnpackPositionalArgs)("abs", args, kwargs, 1, res);
    var x = res[0];
    if (unpackError) {
        return unpackError;
    }
...
function bool_(thread, _, args, kwargs) {
    var x = value_7.False;
    var res = new Array(1);
    var unpackError = (0, unpack_1.UnpackPositionalArgs)("bool", args, kwargs, 0, res);
    if (unpackError) {
        return unpackError;
    }
    x = res[0];
...
function chr(thread, _, args, kwargs) {
    ...
    if (i > 0x10ffff) {
        return new Error("chr: Unicode code point U + ".concat(i.toString(16), " out of range(> 0x10FFFF)"));
    }
...
function getattr(thread, b, args, kwargs) {
    ...
    var obj = res[0], name = res[1], dflt = res[2];
    if ("Attr" in obj && "AttrNames" in obj) {
        var _a = obj.Attr(name), v = _a[0], err_1 = _a[1];
        if (err_1) {
            if (dflt) {
                return dflt;
            }
    ...
    return new Error("getattr: ".concat(obj.Type(), " has no.").concat(name, " field or method"));
}
function hasattr(thread, _, args, kwargs) {
    ...
    var name = res[1];
    if ("Attr" in obj && "AttrNames" in obj) {
        var _a = obj.Attr(name), v = _a[0], err_2 = _a[1];
        if (!err_2) {
...
function print(thread, b, args, kwargs) {
    ...
    console.error("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
    console.error("<<<< print is not impl but i can give you", JSON.stringify(args));
    var sep = " ";
...
function range_(thread, b, args, kwargs) {
    ...
    return new value_9.RangeValue(start, stop, step, (0, value_10.rangeLen)(start, stop, step));
}
