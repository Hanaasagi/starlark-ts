...
function unquote(quoted: string): [string, boolean, boolean, Error | undefined] {
  let s = "";
  let triple = false;
  let isByte = false;
  ...
  while (quoted.length > 0) {
    ...
    buf.push(...quoted.slice(0, i).split("").map((c) => c.charCodeAt(0)));
    ...
    switch (quoted[1]) {
      ...
      case '7':
        ...
        if (!isByte && n > 127) {
          err = new Error(`non-ASCII octal escape \\${n} (use \\u${n.toString(16).padStart(4, '0')} for the UTF-8 encoding of U+${n.toString(16).padStart(4, '0')})`);
          return;
        }
        if (n >= 256) {
          ...
          err = new Error(`invalid escape sequence \\${n.toString(8).padStart(3, '0')}`);
          return;
        }
        buf.push(n);
        break;
      ...
      case 'x':
        ...
        if (!isByte && n > 127) {
          err = new Error(`non-ASCII hex escape ${quoted.slice(0, 4)} (use \\u${n.toString(16).padStart(4, '0')} for the UTF-8 encoding of U+${n.toString(16).padStart(4, '0')})`);
          return;
        }
        buf.push(n);
