...
exports.Scanner = exports.Position = exports.TokenValue = exports.Token = void 0;
...
var FilePortion = /** @class */ (function () {
...
var Position = /** @class */ (function () {
    ...
    function Position(file, line, col) {
        this.file = file;
        this.line = line;
        this.col = col;
    }
    ...
    Position.prototype.filename = function () {
        if (this.file !== null) {
            return this.file;
        }
        return "<invalid>";
    ...
    Position.prototype.add = function (s) {
        if (s.includes("\n")) {
            var n = s.split("\n").length - 1;
            this.line += n;
    ...
    Position.prototype.toString = function () {
        ...
        if (this.line > 0) {
            if (this.col > 0) {
                return "".concat(file, ":").concat(this.line, ":").concat(this.col);
            }
            return "".concat(file, ":").concat(this.line);
        }
        return file;
    };
    Position.prototype.isBefore = function (q) {
        if (this.line !== q.line) {
...
var ScannerError = /** @class */ (function (_super) {
    ...
    function ScannerError(pos, msg) {
        var _this = _super.call(this, "".concat(pos.toString(), ": ").concat(msg)) || this;
        _this.pos = pos;
        _this.msg = msg;
        return _this;
    }
...
var Scanner = /** @class */ (function () {
    ...
    function Scanner(filename, src, keepComments) {
        var firstLine = 1;
        var firstCol = 1;
    ...
    Scanner.prototype.error = function (pos, msg) {
    ...
    Scanner.prototype.peekRune = function () {
        ...
        var result = textDecoder.decode(this.rest.slice(0, 4), { stream: true });
        var r = result[0];
        return r;
    };
    // readRune consumes and returns the next rune in the input.
    // Newlines in Unix, DOS, or Mac format are treated as one rune, '\n'.
    Scanner.prototype.readRune = function () {
        // eof() has been inlined here, both to avoid a call
        // and to establish len(rest)>0 to avoid a bounds check.
        if (this.rest.length === 0 && !this.readLine()) {
            return "\0";
    ...
    Scanner.prototype.startToken = function (val) {
    ...
    Scanner.prototype.endToken = function (val) {
        if (val.raw === "") {
            val.raw = this.token
    ...
    Scanner.prototype.nextToken = function (val) {
        ...
        var blank = false;
        var savedLineStart = this.lineStart;
        if (this.lineStart) {
            ...
            if (!blank && this.depth === 0) {
                var cur = this.indentstk[this.indentstk.length - 1];
                if (col > cur) {
                    // indent
                    this.dents++;
                    ...
                    if (col !== this.indentstk[this.indentstk.length - 1]) {
                        this.error(this.pos, "unindent does not match any outer indentation level");
                    }
                }
            }
        }
        ...
        if (c === "#") {
            ...
            if (this.keepComments) {
                this.endToken(val);
                if (blank) {
                    this.lineComments.push(new syntax_1.Comment(val.pos, val.raw));
                }
                else {
                    this.suffixComments.push(new syntax_1.Comment(val.pos, val.raw));
                }
            }
        }
        // newline
        ...
        switch (c) {
            ...
            case "}":
                if (this.depth == 0) {
                    this.error(this.pos, "unexpected ".concat(c));
                }
                else {
                    this.depth--;
                }
        ...
        this.error(this.pos, "unexpected input character ".concat(c));
    };
    Scanner.prototype.scanString = function (val, quote) {
        var start = this.pos;
        var triple = this.rest.length >= 3 &&
            //BUG:
            //@ts-ignore
            this.rest[0] === quote &&
            this.rest[1] === quote &&
        ...
        var _a = [val.raw, false, null], s = _a[0], isByte = _a[1], err = _a[2];
        // if (err) {
    ...
    Scanner.prototype.scanNumber = function (val, c) {
        var start = this.pos;
        var fraction = false;
        var exponent = false;
        if (c === ".") {
            ...
            if (c === ".") {
                ...
                if (c === ".") {
                    ...
                    this.endToken(val);
                    this.error(this.pos, "obsolete form of octal literal; use 0o".concat(val.raw.slice(1)));
                }
            }
        }
        else {
